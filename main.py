...
def kcarter():
    ...
    next_link = "http://" + config.first_party_trackers[0] + "/results?"
...
@app.route("/results")
def results():
    print(request.args.get('fpi_whorls'))
    ...
    # my modification
    return render_template('results.html',fpi_whorls=quote(request.args.get('fpi_whorls') or {}, safe='') ,third_party_trackers=config.third_party_trackers)
